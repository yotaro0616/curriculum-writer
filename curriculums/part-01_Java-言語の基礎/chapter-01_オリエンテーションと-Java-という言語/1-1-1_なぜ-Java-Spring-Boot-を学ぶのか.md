...
Part 1 では、Java を「読み書きできる」状態になることを目指します。その最初の Chapter となる本章では、本教材全体の地図を手に入れ、Java という言語の正体（なぜコンパイルが必要なのか、JVM とは何か）をつかみ、Java のコードがどんな器（クラス・パッケージ・プロジェクト）に宿るのかまでを見渡します。
