...
3. **プロンプトを構成**: `references/style-guide.md` に従う。**日本語ラベルは短く最小限**に抑える（文字アーティファクト対策）
4. **生成**:
   ```bash
...
5. **目視確認**: Read ツールで画像を開き、(a) 意図した概念が伝わるか (b) 無関係な英単語の混入・文字崩れ・要素過多がないか を確認する。問題があればラベルを減らす等プロンプトを調整して再生成する
...
- 既定は **4K / Pro**。概算で **約 $0.2〜0.25 / 枚**。概念 34 Section をすべて生成すると概算 **$8〜10 程度**（2026-06 時点の目安）
...
- 図の主役は **Mermaid**（writing.md: 図は Mermaid 主体・本文中）。illustrate は 🧠 直後のメンタルモデル 1 枚に限定し、本文の Mermaid を再描画せず別角度（比喩・俯瞰・Before/After）から描く
...
| `references/criteria.md` | 5つの画像タイプと選び方・中心概念の選定 | 中心概念とタイプの選定時 |
