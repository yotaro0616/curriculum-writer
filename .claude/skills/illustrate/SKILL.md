...
Why ブロック配下の 🧠 ブロッククオートの直後、`---` 区切りの直前に挿入する（アンカーは常に 🧠。アークモードがモード2なら「なぜ〇〇を使うのか」配下、モード1なら「導入:」配下にある）。

<!-- /setup で admonition 様式を選んだ場合: 🧠 は !!! quote "現場での考え方" になるため、挿入位置は「quote ブロックの直後・次の --- の直前」と読み替える。quote が無い Section では Why ブロック本文の末尾（--- の直前）に挿入する -->
...
- **コミットはこのパスの成果物のみ**（画像＋プロンプト記録＋挿入タグ。例: `illustrate(Part 2): ...`）。本文執筆・動画の変更と混載しない
