...
| 経路 | 作成手段 | 特徴 |
|------|---------|------|
| **Claude Design（既定）** | ユーザーが claude.ai/design で対話的に作図 → zip 書き出し → `/design-ingest` で取り込み | 部分修正・複数案の比較・デザインシステムによる統一を UI 上で行える。仕上がりを追い込みやすい |
| **genai（選択式）** | Gemini / OpenAI の API でこのスキルが生成 | 一括で枚数を揃えやすい。API キーと生成コストが必要 |

経路の既定は /pilot（writing.md の初期調整）で選択し、`references/criteria.md`「0. 方針の決定記録」に記録する。**Section 単位の上書きも可**（実行時に `--route genai` / `--route claude-design` を付けるか、依頼文で指定する）。

配置は各 Section の導入 🧠（[人格名]はこう考える）直後の **代表図** が基本。密度方針 [C] では `##` 見出し単位の **追加図** も扱う。再実行しても配置済みの図はスキップする（冪等）ため、Part / Chapter を書き終えるたびに繰り返し実行できる。
...
- **密度方針**は `references/criteria.md`「0. 方針の決定記録」で設定する（/pilot で選択）:
  - **[A] 各概念 Section に 1 枚**: スコープ内の全概念 Section に代表図を入れる
  - **[B] 判断ベース（デフォルト）**: criteria.md の観点に強く該当する Section のみ代表図を入れる
  - **[C] 概念アンカーごと**: 代表図に加え、`##` 見出し（概念アンカー）ごとに追加図を検討する（1 Section 複数図）
...
1. **Section ファイルを全文読む**
2. **図にする中心概念を 1 つ選ぶ**: 「how it works（仕組みの正確な図解）」ではなく「how to think about it（直感的な掴み）」を選ぶ。代表図は 🧠 直後に置かれるため、**導入の 🧠 が使っている比喩・痛みに最も呼応する概念** を選ぶ。追加図（[C]）は該当 `##` 見出しの主役概念を選ぶ。選定基準は `references/criteria.md`
...
| --size | 1792x1008 | OpenAI | gpt-image-2 は flexible: 16:9 `1792x1008`・4K `3840x2160` 等（両辺16の倍数）。1.5/mini は `1024x1024`/`1536x1024`/`1024x1536` 固定 |
...
| ファイル | 内容 | いつ読むか |
|---------|------|-----------|
| `references/criteria.md` | 方針の決定記録（経路・密度）・中心概念の選定・3 つの構図・画像の規範（alt・記録・改版） | plan 時・中心概念とタイプの選定時 |
