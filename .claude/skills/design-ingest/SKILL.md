...
規約の正は `/illustrate` SKILL.md「両経路共通の規約」（代表図 = Why ブロックの 🧠 直後・追加図 = 該当 `##` 見出し末尾・emoji 以外の様式と weave の読み替えを含む）。ここには本スキルが実行する挿入アルゴリズムだけを置く。

挿入アルゴリズム（代表図）:

1. Why ブロック配下の 🧠 見出しとそのブロッククオートを探す（emoji 以外の様式では人格コラムのブロック＝admonition: `!!! quote` / plain・zenn: `### [人格名]はこう考える`＋ブロッククオート、を同じアンカーとして読む。🧠 相当が無い Section では Why ブロック本文の末尾、weave の混合では冒頭ブロックの末尾を挿入点とする）
