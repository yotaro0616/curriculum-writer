...
**フェーズ早見表**（各フェーズで何を実行し、ゲートで何を承認するか。以前の単一 `/setup` 一括フローから、フェーズ分割＋ゲート永続化＋決定録の集約に変わりました）:

| フェーズ | 実行 | 成果物 | ゲートで承認するもの |
|---|---|---|---|
| 調査 | `/research`（小規模は `light`） | `RESEARCH.md` | **G1**: 調査結論（トピックマップ・題材候補・読者仮説） |
| 哲学 | `/define` | `CLAUDE.md` | **G2**: WHO / WHY / WHAT / 階層構造 |
| 構造 | `/outline` | `OUTLINE.md`（骨格） | **G3**: Section 構成・ゴール・種類・順序 |
| 試作・様式 | `/pilot` | `PROGRESS.md` の `config`（決定録）＋試作 Chapter | **G4**: 様式ロック＝**量産解禁** |
| （章ごと） | `/outline <Chapter>` | OUTLINE の見出し骨子（JIT 充填） | 執筆直前に充填（バックフィル禁止） |
| 執筆 | `/write <スコープ>` | `curriculums/` の本文 | — |
| レビュー | `/review <スコープ>` | 指摘レポート（自動修正しない） | — |

ゲート承認は `PROGRESS.md` に永続化され、`/write` が量産前に検査します。`/setup` を実行すれば、いまどのゲートにいて次に何をすべきかを常に案内します。
...
レビュー前に共通 lint（`scripts/lint_curriculum.py`）を実行します。同じ検査は執筆時の PostToolUse hook・PR 時の CI でも自動実行されます（手動実行はプロジェクトルートで行ってください: 様式・学習モデルの自動読みがカレントディレクトリの `PROGRESS.md` に依存します）。/revise の適用後は「変更スコープモード」（網羅性・正確性・整合性の3軸）で検収します。
...
Remotion で、静止画では表しにくい「時間軸を持つ説明」（対比の展開・処理の流れ・状態変化）を Section 解説動画（3〜6分・1080p）として生成・挿入します。`/illustrate` の概念図を動画の素材としても再利用します。密度は `/pilot` で設定し、既定は判断ベース（動画が最も効く Section に限定・全概念 Section への生成や主学習路化は選択式）です。
...
特定の企業・組織向けの教材も、汎用教材と同じ **1教材 = 1リポジトリ**・同じフロー（/setup → 上流4フェーズ → /write）で作ります。専用モードや専用ファイルはありません。組織固有の前提は、通常の成果物に収めます。

| 組織固有の前提 | 置き場 |
|---|---|
| 事業・受講者の実像・現場スタック・研修の制約 | `/research` の会話で渡す → `RESEARCH.md` に整理。WHO・技術スタックは `/define` が `CLAUDE.md` に落とす |
| 用語 / NG 用語 | 用語は `.claude/rules/prh.yml`、本文で使わない表現は `PROGRESS.md` の `config.excluded_terms`（決定録） |
| 既存の社内資料がある | `/research` のギャップ分析（Extend / Create New / Restructure）を使う |

> ⚠️ 内部検討資料（価格・競合比較等）や組織固有の生ブリーフは、教材リポジトリに含めず別管理にしてください（public リポジトリに内部資料が残った実例があります）。
...
│   ├── state/                # 実行時状態（check-updates.json。write-briefs/ は gitignore）
